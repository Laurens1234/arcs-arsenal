...
const el = {
  ...
  simulationSims: document.getElementById("simulationSims"),
  simulationSimsDisplay: document.getElementById("simulationSimsDisplay"),
...
function getBestPickSim(playerIdx, sims = 200) {
  ...
  // Adjust sims based on pool size for performance
  const simCount = Math.max(30, Math.min(sims, 500));

  // Run an equal number of sims per candidate
  const simsPerCandidate = Math.max(5, Math.floor(simCount / candidates.length));
  for (const choice of candidates) {
    const key = choice.name + '|' + choice.pickType;
    for (let s = 0; s < simsPerCandidate; s++) {
      ...
      while (safety-- > 0) {
        ...
        const pick = chooseBestPickForAgent(simDraft, pIdx, 3);
        if (!pick) break;
        if (pick.pickType === 'leader') {
          const idx2 = simDraft.availableLeaders.findIndex(x => x.name === pick.name);
          if (idx2 !== -1) simDraft.availableLeaders.splice(idx2, 1);
  ...
  // Choose candidate with highest average final score
  let best = null; let bestAvg = -Infinity;
  for (const c of candidates) {
    const key = c.name + '|' + c.pickType;
    const sum = results.get(key) || 0;
...
function initSetupUI() {
  ...
  if (el.simulationSims) {
    el.simulationSims.addEventListener('input', () => {
      const v = parseInt(el.simulationSims.value, 10);
      draft.simulationSims = v;
      if (el.simulationSimsDisplay) el.simulationSimsDisplay.textContent = String(v);
    });
    // initialize display
    if (el.simulationSimsDisplay) el.simulationSimsDisplay.textContent = String(draft.simulationSims || parseInt(el.simulationSims.value,10));
  }
  // Button groups
...
function getBestPick(playerIdx) {
  if (draft.bestPickAlgo === 'simulation') {
    return getBestPickSim(playerIdx, draft.simulationSims);
  }
